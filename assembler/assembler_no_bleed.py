...
roles = ['Volunteer', 'Jury', 'Organizer'] #['Contestant', 'Coach', 'Jury', 'Organizer', 'Volunteer', 'Sponsor']
...
for role in roles:
    ...
    for i, pos in enumerate(positions):
        os.system(f"pdfjam --scale {scale} --offset '{offset[i]}' --outfile \
                    '{back_dir}/scaled/{role}-{pos}.pdf' --a6paper -- '{back_dir}/{role}.pdf'")

    merge_call = f"pdfjam --nup 2x2 --a4paper --outfile '{output_dir}/{role}-badges-no-bleed.pdf' -- "
    #scaling fronts
    for i, front_file in enumerate(glob.glob(f'{front_dir}/{role}*.pdf')):
        os.system(f"pdfjam --scale {scale} --offset '{offset[i % 4]}' --outfile \
